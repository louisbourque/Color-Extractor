...
function getImageDetails(imageElement){
  ...
  var colorsGradient = [];
  colorsGradient[0] = getPalette(imageElement,0);
  colorsGradient[1] = getPalette(imageElement,1);
  colorsGradient[2] = getPalette(imageElement,2);
  colorsGradient[3] = getPalette(imageElement,3);
  ...
  innerHTML += '<div class="color-background-gradient" style="background-image: linear-gradient(to right, '+colorArrayToHex(colorsGradient[0][0])+' 0%, '+colorArrayToHex(colorsGradient[1][0])+' 50%, '+colorArrayToHex(colorsGradient[2][0])+' 75%, '+colorArrayToHex(colorsGradient[3][0])+' 100%);"><button title="Copy to clipboard" class="copy-button" data-hex="background-image: url(path/to/img.jpg), linear-gradient(to right, '+colorArrayToHex(colorsGradient[0][0])+' 0%, '+colorArrayToHex(colorsGradient[1][0])+' 50%, '+colorArrayToHex(colorsGradient[2][0])+' 75%, '+colorArrayToHex(colorsGradient[3][0])+' 100%);"><svg aria-hidden="true" height="16" version="1.1" viewBox="0 0 14 16" width="14"><path d="M2 13h4v1H2v-1zm5-6H2v1h5V7zm2 3V8l-3 3 3 3v-2h5v-2H9zM4.5 9H2v1h2.5V9zM2 12h2.5v-1H2v1zm9 1h1v2c-.02.28-.11.52-.3.7-.19.18-.42.28-.7.3H1c-.55 0-1-.45-1-1V4c0-.55.45-1 1-1h3c0-1.11.89-2 2-2 1.11 0 2 .89 2 2h3c.55 0 1 .45 1 1v5h-1V6H1v9h10v-2zM2 5h8c0-.55-.45-1-1-1H8c-.55 0-1-.45-1-1s-.45-1-1-1-1 .45-1 1-.45 1-1 1H3c-.55 0-1 .45-1 1z"></path></svg></button>background-image: url(path/to/img.jpg),<br>linear-gradient(to right, '+colorArrayToHex(colorsGradient[0][0])+' 0%, '+colorArrayToHex(colorsGradient[1][0])+' 50%, '+colorArrayToHex(colorsGradient[2][0])+' 75%, '+colorArrayToHex(colorsGradient[3][0])+' 100%);</div>';
